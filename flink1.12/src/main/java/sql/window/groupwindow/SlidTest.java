...
public class SlidTest {
    ...
    public void processtime() throws Exception {
        ...
        //开滚动窗口计算wordCount
        Table select = table.window(Slide.over(lit(5).seconds()).every(lit(2).seconds()).on($("pt")).as("sw"))
                .groupBy($("id"), $("sw"))
                .select($("id"), $("id").count());
    ...
    public void eventTime() throws Exception {
        ...
        //开滚动窗口计算wordCount
        //0-5 5-10, 10,15每隔5s进行一次窗口 到达5s之后 每隔2s执行一次输出，输入时间1 则对应的时间窗口为【-5，0） 【-3，2） 【-1，4）  【1 6）
        Table select = table.window(Slide.over(lit(5L).seconds()).every(lit(2).seconds()).on($("rt")).as("tw"))
                .groupBy($("id"), $("tw"))
                .select($("id"), $("id").count());
    ...
    public void count() throws Exception {
        ...
        //开滚动窗口计算wordCount
        Table select = table.window(Slide.over(rowInterval(5L))
